    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3 text-xs">
            <div class="bg-slate-900 p-3 rounded border border-slate-700">
                ...
                <span class="font-mono text-sky-400">X: {mousePos.x}, Y: {mousePos.y}</span>
            </div>
            <div class="bg-slate-900 p-3 rounded border border-slate-700">
                <span class="block text-slate-500 mb-1">Scroll (Desplazamiento)</span>
                <span class="font-mono text-emerald-400">X: {scrollPos.x.toFixed(0)}, Y: {scrollPos.y.toFixed(0)}</span>
            </div>
            <div class="bg-slate-900 p-3 rounded border border-slate-700">
    ...
    <div class="space-y-4">
        ...
        <div class="relative">
            ...
            <div class="absolute right-3 top-3">
                <span class="w-2 h-2 rounded-full inline-block {keystrokes.length > 0 && performance.now() - keystrokes[keystrokes.length-1] < 200 ? 'bg-green-500 animate-pulse' : 'bg-slate-700'}"></span>
            </div>
        </div>
        
        <div class="flex gap-4 items-center">
             <div class="bg-slate-900 p-3 rounded border border-slate-700 flex-1">
                <span class="block text-slate-500 text-xs mb-1">Latencia Media entre Teclas</span>
