<script lang="ts">
  ...
  import type { ExtendedNavigator } from "../types/types";
      ...
      try {
        ...
        pc.onicecandidate = (e) => {
          ...
          if (match) {
            ...
            if (
              ip.match(
                /^(192\.168\.|169\.254\.|10\.|172\.(1[6-9]|2[0-9]|3[0-1])\.)/,
              )
            ) {
              // Private IP
              displayIp = `🏠 ${ip}`;
...
>
  ...
  <DataRow label="Estado de Red">
    {#snippet value()}
      <div
        class="flex items-center justify-end gap-2 font-bold {isOnline
          ? 'text-emerald-400'
          : 'text-red-400'}"
      >
        <span
    ...
    {#snippet value()}
      {#if ips.length > 0}
        <div class="flex flex-col items-end gap-1">
          {#each ips as ip}
            <span
              class="text-xs bg-slate-900/50 px-2 py-0.5 rounded border border-slate-700/50"
            >
              {ip}
            </span>
          {/each}
  ...
  />
  <DataRow label="Latencia (RTT)" value={rtt ? `${rtt} ms` : "N/A"} />
  <DataRow
...
>
  <div class="space-y-4">
    <div>
      <h4 class="font-semibold text-sky-400 mb-1">
        Si ves una IP que empieza por 192.168.x.x o 10.x.x.x:
      </h4>
      <p class="text-slate-400">
        Tu navegador está informando de tu dirección de red interna. Cualquier
        sitio web que visites podría saber cómo está organizada tu red
        doméstica.
      </p>
    </div>
    ...
    <div>
      <h4 class="font-semibold text-sky-400 mb-1">
        Si ves una cadena larga de letras y números (IPv6):
      </h4>
      <p class="text-slate-400">
        Es tu dirección de nueva generación. Muchas veces estas son temporales,
        pero siguen siendo identificadores únicos.
      </p>
    </div>

    <div>
      <h4 class="font-semibold text-green-400 mb-1">
        Si no aparece nada o da error:
      </h4>
      <p class="text-slate-400">
        ¡Felicidades! Tu navegador o tu VPN están bloqueando con éxito las fugas
        de WebRTC.
      </p>
    </div>
  </div>
